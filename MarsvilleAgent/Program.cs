...
static AgentAction ChooseAction(GameState s)
{
    ...
    // On 2D boards we try E then S; on corridors East only.
    var moveOrder = s.BoardHeight > 1
        ? new[] { 0, 3, 2, 1 }
        : new[] { 0 };

    foreach (var dir in moveOrder)
    {
        var (dx, dy) = DirOffset(dir);
        int nx = s.X + dx, ny = s.Y + dy;

        if (!InBounds(nx, ny, s.BoardWidth, s.BoardHeight)) continue;

        var target = cellMap.GetValueOrDefault((nx, ny));
        if (target is null) continue; // in fog -- skip
        ...
        switch (target.CellType)
        {
            ...
            case "low_obstacle":
                return new AgentAction("crawl", dir);

            case "hole":
                // Jump if landing cell is walkable
                int lx = s.X + dx * 2, ly = s.Y + dy * 2;
                var land = cellMap.GetValueOrDefault((lx, ly));
                if (land is { CellType: "floor" or "bridge" or "goal" })
                    return new AgentAction("jump", dir);
                break;

            case "broken_bridge":
                if (s.Backpack.Contains("plank") && s.Backpack.Contains("nail"))
                    return new AgentAction("build", dir);
                // Step toward visible materials
                var matCell = s.VisibleCells.FirstOrDefault(c =>
                    c.Items.Contains("plank") || c.Items.Contains("nail"));
                if (matCell is not null)
                {
                    int pd = DirectionToward(s.X, s.Y, matCell.X, matCell.Y);
                    if (pd >= 0)
                    {
                        var (mdx, mdy) = DirOffset(pd);
                        var mCell = cellMap.GetValueOrDefault((s.X + mdx, s.Y + mdy));
                        if (mCell is { CellType: "floor" or "bridge" })
                            return new AgentAction("move", pd);
                    }
                }
                break;
        }
    ...
    {
        var (dx, dy) = DirOffset(dir);
        var adj = cellMap.GetValueOrDefault((s.X + dx, s.Y + dy));
        if (adj?.Entity?.EntityType == "enemy")
            return new AgentAction("attack", dir);
    }

    // Still crawling through an obstacle -- continue East
    if (s.IsCrawling)
        return new AgentAction("crawl", 0);

    return new AgentAction("wait", null);
}

static (int dx, int dy) DirOffset(int dir) => dir switch
{
    0 => (1, 0),
    1 => (-1, 0),
    2 => (0, -1),
...
static int DirectionToward(int fx, int fy, int tx, int ty)
{
