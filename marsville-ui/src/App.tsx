...
function App() {
  ...
  const { connected } = useGameHub({
    ...
    onRoundEnded: useCallback((data: { cumulative: Record<string, number> }) => {
      setScores(
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-orange-100 flex flex-col">
      {/* Header */}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-72 flex-shrink-0 overflow-auto p-4 space-y-4 border-l border-orange-900 bg-stone-900">
          {showAdmin && <AdminPanel onRoundStarted={() => setBoards(new Map())} />}
          <Scoreboard scores={scores} />

          {/* Level legend */}
          <div className="bg-stone-800 rounded-lg p-3 border border-orange-900">
            <h3 className="text-orange-300 font-semibold text-sm mb-2">Cell Legend</h3>
            <ul className="space-y-1 text-xs text-orange-200">
              <li><span className="inline-block w-4 h-4 bg-orange-300 rounded mr-1" /> Floor</li>
              <li><span className="inline-block w-4 h-4 bg-gray-900 rounded mr-1" /> Hole</li>
              <li><span className="inline-block w-4 h-4 bg-amber-700 rounded mr-1" /> Broken bridge</li>
              <li><span className="inline-block w-4 h-4 bg-amber-400 rounded mr-1" /> Bridge</li>
              <li><span className="inline-block w-4 h-4 bg-stone-500 rounded mr-1" /> Low obstacle</li>
              <li><span className="inline-block w-4 h-4 bg-green-400 rounded mr-1" /> Goal &#x1F680;</li>
              <li><span className="inline-block w-4 h-4 bg-gray-700 rounded mr-1" /> Wall</li>
              <li><span className="inline-block w-4 h-4 bg-gray-800 rounded mr-1" /> Fog of war</li>
            </ul>
          </div>
        </aside>
      </div>
